...
Fixpoint split_aux {X Y : Type} (l : list (X*Y)) (lx : list X) (ly: list Y) : (list X) * (list Y) :=
  match l with
  | [] => (lx,ly)
  | (x,y) :: l' =>  split_aux l' (x::lx) (y::ly)
  end.

Definition split_rev {X Y : Type} (l : list (X*Y)): (list X) * (list Y) :=
    split_aux l [] [].
...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun l => 7 <? l) (filter even l) .
...
Proof.
  intros X Y f l. 
    induction l as [| n l' IHl'].
  - reflexivity.
  - simpl. rewrite <- IHl'. rewrite -> app_map. simpl. reflexivity.
Qed.
