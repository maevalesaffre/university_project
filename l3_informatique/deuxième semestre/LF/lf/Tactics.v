...
Proof.
  ...
  rewrite H1. assert (H: y :: l = z ::l).
  { rewrite H2. rewrite eq2. reflexivity. }
  injection H as Ha.
  symmetry. apply Ha. Qed.
...
Proof.
  ...
  -  intros m eq. destruct m as [| m'] eqn:E.
    + discriminate eq.
    + apply f_equal. apply IHn'. simpl in eq. rewrite eq. reflexivity. Qed.
...
Proof.
  ...
  - intros m eq. destruct m.
    + discriminate eq.
    + simpl in eq. f_equal. apply IHn'. injection eq as eq.
...
Theorem nth_error_after_last: forall (n : nat) (X : Type) (l : list X),
  ...
  nth_error l n = None.
Proof.
    intros n X l. induction l as [| l' IHl'].
  - simpl. intros eq. destruct n as [| n'] eqn:E.
    + reflexivity.
    + discriminate eq.
  -  intros eq. destruct n as [| n'] eqn:E.
    + discriminate eq. 
    + simpl in eq. f_equal. injection eq as eq.  admit.
...
Proof.
  ...
  - destruct n.
  intros l1 l2 eq. simpl in eq. destruct (split l') as [l1' l2']. injection eq as eq1 eq2. rewrite <- eq1. rewrite <-eq2. simpl. rewrite IHl'.
  + reflexivity.
  + reflexivity. Qed.
