...
#define NB_EXPECTED 2
char* expected_str[NB_EXPECTED] = {"test_cases/arbo_test5/not_link", "test_cases/arbo_test5/link"};
bool found[NB_EXPECTED] = {false, false};
bool failed = false;
...
int main(int argc, char* argv[]) {
    ...
    printf("Running exercice 1 on \"test_cases/arbo_test5/\"\n");

    pid_t p = fork();
    if(p == 0) {
        liste_sous_reps("test_cases/arbo_test5/");
        bool all_found = true;
        ...
        if(all_found && !failed) {
            ...
            ADD_POINT(testfile, "arbo_test5", 2);
        }
